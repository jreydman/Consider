...
public class ReportRepository extends Dao {
    ...
    public String PART1 = "WITH a  AS ("
            ...
            + "n.SHTRIHKOD,"
            + "p.NAME  AS POSTAVKA_NAME_OLD,"
            + "c.PRICEREAL3,IIF((COALESCE(c.VEDKODC,'')!=''),c.VEDKODC,COALESCE(n.VEDKOD,'') ) as VEDKODC,  "
            ...
            //+ "LEFT JOIN POSTAVKA p  ON p.KOD =v.KODPARTNERA  "
            + "LEFT JOIN POSTAVKA p  ON p.KOD =n.KODPOST  "
            + "LEFT JOIN USERS u  ON u.ID =v.BODYDOC  "
            + "WHERE m.TIPDOC = 8  "
            + "AND m.DATADOC  BETWEEN ? AND ? " //cast ('%s' AS date) AND CAST ('%s' AS date) "
            + " AND m.KODSKLADA = ? ";
    //+ " AND %s "
  /*  public String PART2 = "ORDER BY m.NOMKOD ) "
            ...
            + "round(a.kolvo*a.CENARASX,2) AS SUM_REAL, round(COALESCE(c.FIRST_RASX,a.PRICEREAL3)*a.KOLVO,2) AS FIRST_RASX,"
            + "round(COALESCE(c.FIRST_RASX,a.PRICEREAL3),2) as CENA_ROZN, PDV,"
            //+ "CAST(round((a.CENARASX-COALESCE(c.cena_nds,a.PRICEPRIXOD))*a.kolvo,2) AS NUMERIC (15,2)) AS VAL_DOHOD, "
            //////+ "round(round(a.kolvo*a.CENARASX,5)-round(COALESCE(c.cena_nds,a.PRICEPRIXOD)*a.kolvo,5),5) AS VAL_DOHOD, "
            + "round(round(a.kolvo*a.CENARASX,2)-round(COALESCE(c.cena_nds,a.PRICEPRIXOD)*a.kolvo,5),5) AS VAL_DOHOD, "
            + " CASE COALESCE(c.perenos,0) "
            + " WHEN 0 THEN P.NAME "
            + " ELSE 'Перенос с '||l.NAME END AS POSTAVKA_NAME,a.SHTRIHKOD,a.DATADOC,a.VEDKODC,a.STAVKA_NDS,a.FULLNAME,a.CHECKDOPINFO "
            + "FROM a  left  JOIN CENA_REPORT3(NOMKOD) c ON NOMKOD=c.KOD_NOM "
    ...
    public List<Report> getAll(Integer sklad, LocalDate from, LocalDate to, String filter, YesPrint yesPrint, LocalTime start, LocalTime stop) {
        ...
        try {
            ...
            while (true) {
                ...
                List<Report> listPage = runner.query(SQL_ALL, new Object[]{first, skip, from, to, sklad}, beanListHandler);
                skip += first;
                ...
                listPage.forEach(r -> {
                    ...
                    //r.setNDS(Precision.round(r.getSUM_NDS()-r.getSUM_BEZ_NDS(),5))
 ...
 * 
    public String PART1 = "WITH a  AS ("
            + "SELECT FIRST ? SKIP ? m.NOMKOD , m.NAMEKOD , m.DATADOC ,m.KOLVO , n.NAME ,"
            + "m.KODMAT, n.KODKLASS , k.NAME AS KLASS_NAME, n.PDV, case WHEN v.YESPRINT=1 THEN 'Фискальный' else 'Нефискальный' END  as YESPRINT, v.DATESROK, "
            + "m.KODSKLADA ,cast(round(m.CENARASX,2)   as numeric (15,2)) AS CENARASX, CAST(round(c.PRICEPRIXOD,2) AS   numeric (15,2)) AS PRICEPRIXOD, "
            + "n.SHTRIHKOD,p.NAME  AS POSTAVKA_NAME,c.PRICEREAL3,IIF((COALESCE(c.VEDKODC,'')!=''),c.VEDKODC,COALESCE(n.VEDKOD,'') ) as VEDKODC,  "
            + " case (n.PDV) when 0 then '7%' when 1 then '20%' when 2 then '0%' else '-' end as STAVKA_NDS, u.FULLNAME,v.CHECKDOPINFO "
            + "FROM mv m "
            + "JOIN NAME n ON n.KOD =m.NAMEKOD "
            + " JOIN MVC v ON v.KROSSNOM  =m.KROSSNOMER "
    ...
    public String PART2 = "ORDER BY m.NOMKOD ) "
            ...
            + "round(round(a.kolvo*a.CENARASX,2)-round(COALESCE(c.cena_nds,a.PRICEPRIXOD)*a.kolvo,5),5) AS VAL_DOHOD, "
            + " CASE COALESCE(c.perenos,0) "
            + " WHEN 0 THEN a.POSTAVKA_NAME "
            + " ELSE 'Перенос с '||l.NAME END AS POSTAVKA_NAME,a.SHTRIHKOD,a.DATADOC,a.VEDKODC,a.STAVKA_NDS,a.FULLNAME,a.CHECKDOPINFO "
            + "FROM a  left  JOIN CENA_REPORT(NOMKOD) c ON NOMKOD=c.KOD_NOM "
            + "LEFT JOIN listskl l ON l.KOD  = c.perenos "
            + "ORDER BY NAME,LAST_DATE ";
