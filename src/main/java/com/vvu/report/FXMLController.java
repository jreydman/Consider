package com.vvu.report;

import com.vvu.report.model.*;
import com.vvu.report.repos.*;
import com.vvu.report.service.ExportPivotService;
import com.vvu.report.service.ExportService;
import com.vvu.report.service.PivotService;
import com.vvu.report.service.ReportService;
...
public class FXMLController implements Initializable {
    ...
    private void onReportAction(ActionEvent event) {
        if (reportStart.getValue().isAfter(reportStop.getValue())) return;
        ...
        if (filterName.size() > 0) {
            ...
            filter.append(String.join(",", filterName.stream().map(a -> Integer.toString(a.getKOD())).toArray(String[]::new)));
            filter.append(") ");
        }
        if (filterKlass.size() > 0) {
            filter.append(" AND k.KOD  IN ( ");
            filter.append(String.join(",", filterKlass.stream().map(a -> Integer.toString(a.getKOD())).toArray(String[]::new)));
            filter.append(") ");
        }

        if (filterGrup.size() > 0) {
            filter.append(" AND  n.KODGROUP IN ( ");
            filter.append(String.join(",", filterGrup.stream().map(a -> Integer.toString(a.getKOD())).toArray(String[]::new)));
            filter.append(") ");
        }
        if (filterVid.size() > 0) {
        ...
        if (filterTip.size() > 0) {
            ...
            filter.append(String.join(",", filterTip.stream().map(a -> Integer.toString(a.getKOD())).toArray(String[]::new)));
            filter.append(") ");
        }
        if (filterPostavka.size() > 0) {
            filter.append(" AND  n.KODPOST IN ( ");
            filter.append(String.join(",", filterPostavka.stream().map(a -> Integer.toString(a.getKOD())).toArray(String[]::new)));
            filter.append(") ");
        }
        ...
        reportService.startService(reportStart.getValue(), reportStop.getValue(), reportGroup.isSelected(), filter.toString(), yesPrint,start,stop);
    ...
    private void onPivotAction(ActionEvent event) {
        if (filterSklad.size() == 0) {
            ...
            alert.setContentText("Не выбраны склады!");
    ...
    private void onExportPivot(ActionEvent event) {
        ...
        if (pivotList.size() == 0) {
            ...
            alert.setTitle("Ошибка");
            alert.setHeaderText("Обратите внимание:");
            alert.setContentText("Нечего экспортировать!");
    ...
    private void applySklad(ActionEvent event) {
        ...
        final BigDecimal total_kolvo = new BigDecimal(selectedReport.stream().mapToDouble(o->o.getKOLVO()).sum());
        tReport.getColumns().get(5).setText("Реализация  Кол-во\n-----\n"
                +total_kolvo.setScale(2,BigDecimal.ROUND_HALF_EVEN).doubleValue());

        final BigDecimal total_sum_bez_nds = new BigDecimal(selectedReport.stream().mapToDouble(o->o.getSUM_BEZ_NDS()).sum());
        tReport.getColumns().get(7).setText("Сумма по приходу без НДС (Закупка)\n-----\n"
                +total_sum_bez_nds.setScale(2,BigDecimal.ROUND_HALF_EVEN).doubleValue());

        final BigDecimal total_sum_nds = new BigDecimal(selectedReport.stream().mapToDouble(o->o.getSUM_NDS()).sum());
        tReport.getColumns().get(8).setText("Сумма по приходу с НДС (Закупка)\n-----\n"
                +total_sum_nds.setScale(2,BigDecimal.ROUND_HALF_EVEN).doubleValue());

        final BigDecimal total_sum_real = new BigDecimal(selectedReport.stream().mapToDouble(o->o.getSUM_REAL()).sum());
        tReport.getColumns().get(9).setText("Сумма реализации всего розница\n-----\n"
                +total_sum_real.setScale(2,BigDecimal.ROUND_HALF_EVEN).doubleValue());

        final BigDecimal total_val_dohod = new BigDecimal(selectedReport.stream().mapToDouble(o->o.getVAL_DOHOD()).sum());
        tReport.getColumns().get(10).setText("Валовый доход  (наценка розница-закупка с НДС)\n-----\n"+
                total_val_dohod.setScale(2,BigDecimal.ROUND_HALF_EVEN).doubleValue());

        final BigDecimal total_nds = new BigDecimal(selectedReport.stream().mapToDouble(o->o.getNDS()).sum());
        tReport.getColumns().get(18).setText("НДС\n-----\n"
                +total_nds.setScale(2,BigDecimal.ROUND_HALF_EVEN).doubleValue());

        final BigDecimal total_discount = new BigDecimal(selectedReport.stream().mapToDouble(o->o.getDISCOUNT()).sum());
        tReport.getColumns().get(22).setText("Скидка\n-----\n"
                +total_discount.setScale(2,BigDecimal.ROUND_HALF_EVEN).doubleValue());
    ...
    private void onExport(ActionEvent event) {
        int size = 0;
        for (List<Report> report : map.values()) {
            size += report.size();
        }
        if (size == 0) {
            Alert alert = new Alert(AlertType.INFORMATION);
    ...
    private void addName(ActionEvent event) {
        ...
        set.addAll(filterName.stream().collect(toList()));
        set.addAll(listName.getSelectionModel().getSelectedItems());
        filterName.clear();
        filterName.addAll(set);

        filterName.sort(new Comparator<Name>() {
            @Override
            public int compare(Name u1, Name u2) {
    ...
    private void addVid(ActionEvent event) {
        ...
        filterVid.sort(new Comparator<Vid>() {
            @Override
            public int compare(Vid u1, Vid u2) {
                return u1.getNAME().compareTo(u2.getNAME());
            }
        });
    }
    ...
    private void delVid(ActionEvent event) {
        ...
        vidList.sort(new Comparator<Vid>() {
            @Override
            public int compare(Vid u1, Vid u2) {
                return u1.getNAME().compareTo(u2.getNAME());
            }
    ...
    private void clearVid(ActionEvent event) {
        List<Vid> selected = filterVid.stream().collect(toList());
        vidList.addAll(selected);
        filterVid.removeAll(selected);
        vidList.sort(new Comparator<Vid>() {
            @Override
    ...
    private void addTip(ActionEvent event) {
        ...
        List<Tip> selected = listTip.getSelectionModel().getSelectedItems();
    ...
    private void clearTip(ActionEvent event) {
        List<Tip> selected = filterTip.stream().collect(toList());
        tipList.addAll(selected);
        filterTip.removeAll(selected);
        tipList.sort(new Comparator<Tip>() {
            @Override
    ...
    private void delKlass(ActionEvent event) {
        ObservableList<Klass> selected = listSelKlass.getSelectionModel().getSelectedItems();
        filterName.removeAll(selected);
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        tReport.setPlaceholder(new Label("Нет данных"));
        tReport.setItems(selectedReport);
        ...
        reportService.setOnSucceeded(new EventHandler<WorkerStateEvent>() {
            ...
            public void handle(WorkerStateEvent event) {
                progressBar.progressProperty().unbind();
                reportStatus.textProperty().unbind();
                map = (Map<Sklad, List<Report>>) event.getSource().getValue();
                selectedReport.clear();
                selectedReport.addAll(map.get(selectedSklad));
                tReport.setItems(selectedReport);
        ...
        pivotService.setOnSucceeded(new EventHandler<WorkerStateEvent>() {
            ...
            public void handle(WorkerStateEvent event) {
                ...
                btExportPivot.setDisable(false);
                pivotList.clear();
                List<PivotRow> list = (List<PivotRow>) event.getSource().getValue();
                ...
                TableColumn<PivotRow, String> name_ = new TableColumn<>("Наименование");
                name_.setCellValueFactory(new PropertyValueFactory<PivotRow, String>("NAME"));
                tPivot.getColumns().add(name_);
                TableColumn<PivotRow, String> klass_ = new TableColumn<>("Производитель");
                ...
                for (int i = 0; i < listSklad.length; i++) {
                    final int j = i;
                    final BigDecimal total_kolvo= new BigDecimal(list.stream().mapToDouble(o->o.getSkladPivots()[j].getKOLVO()).sum());
                    TableColumn<PivotRow, Double> kol = new TableColumn("Количество (" + listSklad[i].getNAME() + ")\n-------------\n"
                            +total_kolvo.setScale(2,BigDecimal.ROUND_HALF_EVEN).doubleValue());
                    kol.setCellValueFactory(new Callback<CellDataFeatures<PivotRow, Double>, ObservableValue<Double>>() {
                        @Override
                    ...
                    final BigDecimal total_sum_bez_nds= new BigDecimal(list.stream().mapToDouble(o->o.getSkladPivots()[j].getSUM_BEZ_NDS()).sum());
                    TableColumn<PivotRow, Double> sumBezNds = new TableColumn("Продаж у закупівельних цінах, без ПДВ\n-------------\n"
                            +total_sum_bez_nds.setScale(2,BigDecimal.ROUND_HALF_EVEN).doubleValue());
                    sumBezNds.setCellValueFactory(new Callback<CellDataFeatures<PivotRow, Double>, ObservableValue<Double>>() {
                        @Override
                        public ObservableValue<Double> call(CellDataFeatures<PivotRow, Double> cd) {
                            return new SimpleObjectProperty<>(cd.getValue().getSkladPivots()[j].getSUM_BEZ_NDS());
                        }
                    });
                    ...
                    final BigDecimal total_sum_nds= new BigDecimal(list.stream().mapToDouble(o->o.getSkladPivots()[j].getSUM_NDS()).sum());
                    TableColumn<PivotRow, Double> sumNds = new TableColumn("Продаж у закупівельних цінах, з ПДВ\n-------------\n"
                            +total_sum_nds.setScale(2,BigDecimal.ROUND_HALF_EVEN).doubleValue());
                    sumNds.setCellValueFactory(new Callback<CellDataFeatures<PivotRow, Double>, ObservableValue<Double>>() {
                        @Override
                        public ObservableValue<Double> call(CellDataFeatures<PivotRow, Double> cd) {
                    ...
                    final BigDecimal total_sum_real = new BigDecimal(list.stream().mapToDouble(o->o.getSkladPivots()[j].getSUM_REAL()).sum());
                    TableColumn<PivotRow, Double> sumReal = new TableColumn("Сумма продажу в цінах реалізації\n-------------\n"
                            +total_sum_real.setScale(2,BigDecimal.ROUND_HALF_EVEN).doubleValue());
                    sumReal.setCellValueFactory(new Callback<CellDataFeatures<PivotRow, Double>, ObservableValue<Double>>() {
                        @Override
                        public ObservableValue<Double> call(CellDataFeatures<PivotRow, Double> cd) {
                    ...
                    final BigDecimal total_first_rasx = new BigDecimal(list.stream().mapToDouble(o->o.getSkladPivots()[j].getFIRST_RASX()).sum());
                    TableColumn<PivotRow, Double> sumFirst = new TableColumn("Сумма продажу в роздрібних цінах\n-------------\n"
                            +total_first_rasx.setScale(2,BigDecimal.ROUND_HALF_EVEN).doubleValue());
                    sumFirst.setCellValueFactory(new Callback<CellDataFeatures<PivotRow, Double>, ObservableValue<Double>>() {
                        @Override
                        public ObservableValue<Double> call(CellDataFeatures<PivotRow, Double> cd) {
                            return new SimpleObjectProperty<>(cd.getValue().getSkladPivots()[j].getFIRST_RASX());
                        }
                    });
                    ...
                    final BigDecimal total_val_dohod = new BigDecimal(list.stream().mapToDouble(o->o.getSkladPivots()[j].getVAL_DOHOD()).sum());
                    TableColumn<PivotRow, Double> valDoh = new TableColumn("Валовий прибуток\n-------------\n"
                            +total_val_dohod.setScale(2,BigDecimal.ROUND_HALF_EVEN).doubleValue());
                    valDoh.setCellValueFactory(new Callback<CellDataFeatures<PivotRow, Double>, ObservableValue<Double>>() {
                        @Override
                        public ObservableValue<Double> call(CellDataFeatures<PivotRow, Double> cd) {
                ...
                final BigDecimal total_itog_sum_bez_nds = new BigDecimal(list.stream().mapToDouble(o->Arrays.stream(o.getSkladPivots()).mapToDouble(a->a.getSUM_BEZ_NDS()).sum()).sum());
                TableColumn<PivotRow, Double> SUM_BEZ_NDS
                        = new TableColumn<>("Ітого продаж у закупівельних цінах, без ПДВ\n-------------\n"
                        +total_itog_sum_bez_nds.setScale(2,BigDecimal.ROUND_HALF_EVEN).doubleValue());
                ...
                final BigDecimal total_itog_sum_nds= new BigDecimal(list.stream().mapToDouble(o->Arrays.stream(o.getSkladPivots()).mapToDouble(a->a.getSUM_NDS()).sum()).sum());
                TableColumn<PivotRow, Double> SUM_NDS
                        = new TableColumn<>("Ітого продаж у закупівельних цінах з ПДВ\n-------------\n"
                        +total_itog_sum_nds.setScale(2,BigDecimal.ROUND_HALF_EVEN).doubleValue());
                ...
                final BigDecimal total_itog_sum_real = new BigDecimal(list.stream().mapToDouble(o->Arrays.stream(o.getSkladPivots()).mapToDouble(a->a.getSUM_REAL()).sum()).sum());
                TableColumn<PivotRow, Double> SUM_REAL
                        = new TableColumn<>("Ітого сумма продажу в цінах реалізації\n-------------\n"
                        +total_itog_sum_real.setScale(2,BigDecimal.ROUND_HALF_EVEN).doubleValue());
                ...
                final BigDecimal total_itog_first_rasx = new BigDecimal(list.stream().mapToDouble(o->Arrays.stream(o.getSkladPivots()).mapToDouble(a->a.getFIRST_RASX()).sum()).sum());
                TableColumn<PivotRow, Double> FIRST_RASX = new TableColumn<>("Ітого сумма продажу в роздрібних цінах\n-------------\n"
                        +total_itog_first_rasx.setScale(2,BigDecimal.ROUND_HALF_EVEN).doubleValue());
                ...
                final BigDecimal total_itog_val_dohod= new BigDecimal(list.stream().mapToDouble(o->Arrays.stream(o.getSkladPivots()).mapToDouble(a->a.getVAL_DOHOD()).sum()).sum());
                TableColumn<PivotRow, Double> VAL_DOHOD
                        = new TableColumn<>("Ітого валовий прибуток\n-------------\n"
                        +total_itog_val_dohod.setScale(2,BigDecimal.ROUND_HALF_EVEN).doubleValue());
